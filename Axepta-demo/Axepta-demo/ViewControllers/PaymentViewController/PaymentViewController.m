...
@interface PaymentViewController ()<AxeptaCheckoutViewControllerDelegate, AxeptaApplePayDelegate, ApplePayTableViewCellDelegate, UITableViewDelegate, UITableViewDataSource, UIPickerViewDelegate, UIPickerViewDataSource>
...
/* Checkout */
@property (strong, nonatomic) AxeptaCheckout *checkout;

/* Apple Pay */
@property (strong, nonatomic) AxeptaApplePay *applePay;
...
- (void)viewDidAppear:(BOOL)animated
{
     ...
     *
     */
    self.checkout = [[AxeptaCheckout alloc] init];
    
    /**
     * Initialization of Apple Pay
     *
     */
    self.applePay = [[AxeptaApplePay alloc] init];
...
-(void)getPaymentMethod{
    ...
    [[Axepta sharedInstance] paymentMethodsOnSuccess:^(NSArray<AxeptaPaymentMethod *> *paymentMethods)
     {
...
#pragma mark - AxeptaCheckoutViewControllerDelegate -

/**
 *
 **/
- (void)checkoutDidAuthorizePaymentForPaymentData:(id<AxeptaPaymentDataProtocol>)paymentData withResponse:(AxeptaPaymentRespose *)response
{
 ...
 *
 **/
- (void)checkoutDidFailToAuthorizePaymentForPaymentData:(id<AxeptaPaymentDataProtocol>)paymentData withError:(NSError *)error withResponse:(AxeptaPaymentRespose *)response
{
...
#pragma mark - AxeptaApplePayDelegate -
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if((indexPath.row < [self numberOfPaymentMethods]-3))
    {
        ...
        [self.checkout instantiateCheckoutViewControllerWithPaymentMethod:paymentMethod
                                                                onSuccess:^(AxeptaCheckoutViewController *checkoutViewController) {
        ...
        AxeptaPayPal *payPal = [[AxeptaPayPal alloc] init];
...
- (void)onApplePay
{
    AxeptaPaymentMethod *applePayPaymentMethod = [[Axepta sharedInstance] paymentMethodForID: @"pm_applepay"];
...
- (NSArray<PKPaymentSummaryItem *> *)paymentSummaryItemsForItems:(NSArray *)paymentItems
{
    if( !((paymentItems != nil) && (paymentItems.count > 0)) )
    {
        ...
        AxeptaPaymentMethod *applePayPaymentMethod = [[Axepta sharedInstance] paymentMethodForID: @"pm_applepay"];
        AxeptaPaymentParam *applePayPaymentDataParamOrderDesc = [applePayPaymentMethod.paymentData paymentParamWithKey: @"OrderDesc"];
        AxeptaPaymentParam *applePayPaymentDataParamAmount    = [applePayPaymentMethod.paymentData paymentParamWithKey: @"Amount"];
